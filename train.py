...
from agent import ReplayBuffer, VDNLearner
...
def train(
    ...
    learner = VDNLearner(
        n_agents=env.N_AGENTS, obs_dim=env.obs_dim, n_actions=env.N_ACTIONS
    )
    ...
    if exploration_kind == "independent":
        ...
        strategy = CorrelatedEpsilonGreedy(env.N_AGENTS, env.N_ACTIONS, rng=rng)
    else:
