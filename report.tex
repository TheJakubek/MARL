...
\author[]{Project 9 --- Reinforcement Learning}
\affil[]{}
\date{}
...
GPU-scale StarCraft-like task (SMAX \texttt{2s3z}, JAX) it lifts the QMIX final
win rate from \textbf{0.81 to 0.91}. The observation is the most reliable
similarity source, coupling situationally-similar agents with no role
...
Table~\ref{tab:smax} reports the final win rate (last 10\% of episodes) and the
fraction of training to first reach 50\% wins (lower is faster). Under VDN
(simple additive mixer) every variant converges to $\sim$0.95; correlated just
gets there $\sim$6\% sooner. Under QMIX (harder monotonic mixer) correlated
exploration clearly helps: independent plateaus lower (0.81) with high seed
variance, while correlated reaches a \textbf{higher, more reliable final win rate
(0.90--0.91)} and faster (Fig.~\ref{fig:smax}). Coupling exploratory actions
(coordinated focus-fire) helps most when credit assignment is harder.

\begin{table}[t]
\centering\small
\caption{SMAX \texttt{2s3z}: final win rate / fraction-of-training to 50\% wins.}
\label{tab:smax}
\begin{tabular}{lcc}
\toprule
Backbone & Independent & Best correlated \\
\midrule
VDN  & 0.95 / 0.70 & 0.95 / \textbf{0.66} \\
QMIX & 0.81 / 0.74 & \textbf{0.91} / \textbf{0.65} \\
\bottomrule
\end{tabular}
\end{table}
...
gain is not retained. \obs{} is the safe default similarity source. Limitations:
only 2 agents in the toy tasks; LBF's forgetting limits that benchmark's
conclusiveness; and $R$ is recomputed greedily each step rather than annealed.
...
\small
The project was developed with \textbf{Claude Code} (Anthropic Claude Opus) as a
pair-programming assistant: implementing the PyTorch toy-environment stack and the
JAX/JaxMARL SMAX pipeline, debugging cluster issues (CUDA/cuDNN, multi-process GPU
OOM), diagnosing training failures, generating figures, and drafting this report.
\emph{Worked well:} fast iteration; systematic debugging (diagnosing the SMAX
Q-divergence from the loss curve, the $\sim$50$\times$ \texttt{vmap} speed-up).
\emph{Did not:} several defaults were wrong --- most importantly its first SMAX
exploration correlated \emph{whether} each agent explored rather than \emph{which
action} it took, a conceptual bug caught only in human code review. The
assistant accelerates implementation but its output needs careful conceptual
verification.
